...
先量才知道問題不在哪：改動前 **ClickHouse 只佔 21.4s，7%**——被評審看的那部分
本來就快，慢的是我們自己排成一直線的東西。三項改動，依效益排序：
...
**改完之後 ClickHouse 佔比反而變高，這是對的。** 最新一輪 committed artifact
（`docs/m2-greenlight-run.json`）是 **29.5s / 126.7s = 23%**。資料庫的絕對時間
幾乎沒動（21.4s → 29.5s，本來就隨 agent 這輪決定查幾次而浮動），
變的是分母：其他東西砍半了，所以同一段資料庫時間佔了三倍的比例。
這正是這次優化該有的樣子——拿掉的是模型延遲與序列化，不是查詢。
